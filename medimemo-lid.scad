DoPart = "body"; // [all, body, letters]

// These are only cosmetics for the OpenSCAD rendering
BodyColour = "red";
LettersColour = "blue";
...
module DefinePart(Part)
{
    if (DoPart == "all" || DoPart == Part)
    {
        children();
    }
}
...
DefinePart("letters")
{
    ...
    scale([1, stretch, 1])
    rotate([0, 0, -90])
    text(label, halign="right", valign="baseline", size=top_width/2 - label_border, font=font);
}
